#!/usr/bin/env python3
"""Run one bounded, read-only Cursor Agent review.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", type=model_value, help="optional Cursor model token")
...
def validate_response(process_result: ProcessResult) -> str:
    """Validate Cursor's native one-shot result envelope fail-closed."""
    ...
    if payload.get("type") != "result":
        raise RuntimeError("provider type is not result")
    if payload.get("subtype") != "success":
        raise RuntimeError("provider subtype is not success")
    if payload.get("is_error") is not False:
        raise RuntimeError("provider result is marked as error")
    if _nonempty_field(payload, "error"):
        raise RuntimeError("provider error")
    if _nonempty_field(payload, "errors"):
        raise RuntimeError("provider errors")
    if _nonempty_field(payload, "denied_actions"):
        raise RuntimeError("denied actions")
    response = payload.get("result")
    if not isinstance(response, str):
        raise RuntimeError("result is not a string")
...
def _receipt(
    *,
    ...
    verification_gaps = ["Cursor JSON does not report actual model identity"]
    if model == PROVIDER_DEFAULT:
        verification_gaps.insert(0, "provider-selected model unverified")
    receipt: dict[str, Any] = {
        "provider": "Cursor",
        "version": version,
        "model": model,
        "model_source": "provider default (unverified)" if model == PROVIDER_DEFAULT else "requested",
...
def run(args: argparse.Namespace) -> int:
    ...
    command = ["agent", "-p", "--mode", "ask", "--output-format", "json"]
    if args.model:
        command.extend(("--model", args.model))
    command.append(prompt)

    version_result = run_process(["agent", "--version"], min(args.timeout, 5))
    ...
    if version_result.returncode != 0 or version_result.timed_out or not version:
        receipt = _receipt(
            ...
            failure="version preflight failed",
            **context,
        )
        _write_artifact(output_dir / "receipt.json", json.dumps(receipt, indent=2) + "\n")
        print(f"Cursor version preflight failed; artifacts: {output_dir}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"Cursor review failed ({failure}); artifacts: {output_dir}", file=sys.stderr)
        return 1
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Cursor review input error: {exc}", file=sys.stderr)
        return 2
    except OSError as exc:
        print(f"Cursor review output error: {exc}", file=sys.stderr)
        return 2
