...
description: Implementation agent spawned by /si (and /si-quick) for ONE scoped work item from a resolved task. Use the execution
  context selected by the orchestrator and return a structured JSON result.
context: fork
model: opus
allowed-tools:
- Read
