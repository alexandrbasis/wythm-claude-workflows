...
description: Dispatched by /sr ONLY in deep (thermo) scope for a whole-module structural quality audit — abstraction health,
  dramatic simplification (code-judo), file-size, spaghetti growth, type/boundary cleanliness, canonical-layer ownership.
  Reviews beyond the diff (changed files + their module + same-layer siblings). Not for direct invocation — use /sr deep.
tools: Glob, Grep, Read, Edit
model: opus
skills:
- claudops:review-conventions
- claudops:architecture-language
---
