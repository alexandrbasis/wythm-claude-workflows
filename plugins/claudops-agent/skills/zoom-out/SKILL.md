...
description: Step up one layer of abstraction and produce a map of the relevant modules and their callers — used when you
  ...
  lost in this code", or "what calls this". Use as a standalone skill only when explicitly requested by the user; automatic
  selection is not authorization. Portable clients may not enforce this host-level restriction.
metadata:
