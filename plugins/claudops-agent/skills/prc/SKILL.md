---
name: prc
description: Review and address code review comments on PR. Use when asked to 'address review comments', 'fix PR feedback',
  'handle review comments', 'respond to PR comments', or 'address CR comments'. NOT for initiating code review (use /sr),
  NOT for task-based implementation with CR mode (use /si).
allowed-tools: Bash Read Edit Glob Grep AskUserQuestion TodoWrite
metadata:
  claude_argument_hint: '["pr-number"]'
...
> If `.claude/skills/prc/SKILL.md.disabled` exists, stop before reading a fallback.
> **Project configuration:** If the current project contains `.claude/skills/prc/SKILL.md`, read and apply it instead of this bundled default. The project copy is the capability source of truth.
