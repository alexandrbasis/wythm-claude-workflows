---
name: fci
description: Fix CI pipeline failures blocking PR merge. Use when CI checks fail, 'pipeline broken', 'build failing', 'fix
  CI', or 'checks not passing'. NOT for debugging runtime bugs (use /dbg), NOT for code review (use /sr). Use as a standalone
  skill only when explicitly requested by the user; automatic selection is not authorization. Portable clients may not enforce
  this host-level restriction.
allowed-tools: Bash Read Edit Write Grep Glob
metadata:
  claude_argument_hint: '["workflow-name or run-id"]'
