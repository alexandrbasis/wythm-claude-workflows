...
name: cursor-cli
description: >-
  Run Cursor CLI for one-shot cross-AI code review when a Cursor perspective is
  explicitly wanted. Invoke ONLY when the user asks ('cursor review', 'ask
  cursor', 'run cursor'), or another skill explicitly delegates to Cursor. Do
  not invoke it implicitly for a generic review. Not for interactive conversations.
allowed-tools:
  - Bash
  - Read
---
