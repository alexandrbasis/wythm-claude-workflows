#!/bin/bash
# WebFetch Cache (pre) — Serve unchanged docs from cache instead of refetching.
#
# Event:     PreToolUse
# Matcher:   WebFetch
# Blocking:  Conditional (exit 2 on cache HIT to deliver cached body; exit 0 otherwise)
...
# ETag or Last-Modified are never cached (can't revalidate) — so stale docs
# are impossible: the origin decides freshness, not a timer.
#
...
#
# Pairs with webfetch-cache-post.sh (PostToolUse WebFetch), which writes the
# cache entries this hook reads.
#
# Configuration:
...
{
  printf '[webfetch-cache] Cache hit for %s\n\n' "$URL"
