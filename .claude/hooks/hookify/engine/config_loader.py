...
def load_rules(event: Optional[str] = None, cwd: Optional[str] = None) -> List[Rule]:
    ...
    Args:
        ...
        cwd: Optional working directory (uses CLAUDE_PROJECT_DIR env var if not provided)
    ...
    # Find all rule files in .claude/hooks/hookify/rules/ directory
    # Project-local structure: .claude/hooks/hookify/rules/*.local.md
    rules_dir = os.path.join(project_dir, '.claude', 'hooks', 'hookify', 'rules')
    pattern = os.path.join(rules_dir, '*.local.md')
    files = glob.glob(pattern)

    # Also support legacy location for backwards compatibility
    legacy_pattern = os.path.join(project_dir, '.claude', 'hookify.*.local.md')
