...
**This project uses a local copy of hookify** (migrated from global plugin) located at `.claude/hookify/`.

## How It Works

### 1. Hook System

Hookify hooks run on these events:
- **PreToolUse**: Before any tool runs (Bash, Edit, Write, etc.)
- **PostToolUse**: After a tool runs
- **Stop**: When Claude wants to stop working
- **UserPromptSubmit**: When user submits a prompt

These hooks read configuration files from `.claude/hookify/rules/*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.claude/hookify/rules/{rule-name}.local.md` files:
...
Create `.claude/hookify/rules/my-rule.local.md` with the format above.
...
Did you run tests before finishing? Make sure `npm test` or equivalent was run.
```
...
**No Restart Needed**: Hookify rules take effect immediately on the next tool use.

**Block or Warn**: Rules can either `block` operations (prevent running) or `warn` (show message but allow). Set `action: block` or `action: warn`.

**Rule Files Location**: `.claude/hookify/rules/*.local.md` - git-ignored by default.
...
```
.claude/hookify/
├── engine/           # Rule loading and evaluation logic
├── hooks/            # Hook scripts (pretooluse.py, stop.py, etc.)
├── rules/            # Your rule files (*.local.md)
...
**Hook not triggering:**
- Check rule file is in `.claude/hookify/rules/` directory
...
2. Try to trigger it - you should see the warning

3. Refine the rule by editing `.claude/hookify/rules/*.local.md`

4. Create more rules as you encounter unwanted behaviors

For detailed documentation, see `.claude/hookify/docs/hookify-guide.md`
