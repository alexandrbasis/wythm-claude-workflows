...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".claude/hookify/rules/*.local.md"
   ```
...
**Event**: bash
**Pattern**: `rm\s+-rf`
**Message**: "Warning: Dangerous rm command detected!..."

**Status**: Active
**File**: .claude/hookify/rules/dangerous-rm.local.md
```
...
2. Or manually create `.claude/hookify/rules/my-rule.local.md` files
