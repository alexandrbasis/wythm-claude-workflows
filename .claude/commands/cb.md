...
1) Resolve Linear issue by number using `cc-linear` skill:
   ```bash
   cc --mcp-config .claude/mcp/linear.json -p "Get issue WYT-[number] details. Return: title, labels (as comma-separated list)"
   ```
   **Reference**: See `.claude/skills/cc-linear/SKILL.md` for self-contained prompt patterns
